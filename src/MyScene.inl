...
void Scene::init(void){
    ...
    node["table top"] -> modeltransforms.push_back( translate(vec3(0.0f,-0.1f,0.0f)) * scale(vec3(1.7f,0.2f,1.7f)) * rotate( 70.0f*float(M_PI)/180.0f, vec3(0.0f, 2.0f, 0.0f) ) );
    node["table top"] -> childnodes.push_back( node["teapot1"] );
    node["table top"] -> childtransforms.push_back( translate(vec3(-0.5f,0.0f,0.3f)) );
    node["table top"] -> childnodes.push_back( node["teapot2"] );
    node["table top"] -> childtransforms.push_back( translate(vec3( 0.5f,0.0f,0.2f)) * rotate( -120.0f*float(M_PI)/180.0f, vec3(0.0f, 1.0f, 0.0f) ) );
    float position_z = 0.0f;
    float falling = -0.2f;
    float position_x = -0.8f;
    int count = 1;
    for(int i=0; i<20; i++){
        if(i<4){
            node["table top"] -> childnodes.push_back( node["sphere"] );
            node["table top"] -> childtransforms.push_back( translate(vec3( -0.5f,0.1f,position_z)) * scale(vec3(0.1f)));   
            position_z -= 0.25;
            continue;
        }
        node["table top"] -> childnodes.push_back( node["sphere"] );
        node["table top"] -> childtransforms.push_back( translate(vec3( position_x,falling,position_z)) * scale(vec3(0.1f)));
        falling -= 0.2;
        position_z -= pow(0.1,count);
        position_x -= pow(0.1,count);
        count += 1;
    }
    
    node["teapot1"] -> models.push_back( model["teapot1"] );
    node["teapot1"] -> modeltransforms.push_back( scale(vec3(0.3f)) );
    node["teapot2"] -> models.push_back( model["teapot2"] );
    node["teapot2"] -> modeltransforms.push_back( scale(vec3(1.0f,1.5f,1.0f)) * scale(vec3(0.3f)) );
    
    node["bunny"] -> models.push_back( model["bunny"] );
    node["bunny"] -> modeltransforms.push_back( scale(vec3(0.5f)) * translate(vec3(0.0f,1.0f,0.0f)) );
    
    node["sphere"] -> models.push_back( model["sphere"] );
    node["sphere"] -> modeltransforms.push_back(translate(vec3(1.0f,0.0f,0.0f)) );
    
    
    node["world"] -> childnodes.push_back( node["table"] );
    node["world"] -> childtransforms.push_back( translate(vec3(0.0f,0.0f,0.0f)) * rotate( 90.0f*float(M_PI)/180.0f, vec3(0.0f, 1.0f, 0.2f)));
    node["world"] -> childnodes.push_back( node["bunny"] );
    node["world"] -> childtransforms.push_back( translate(vec3(1.0f,0.0f,0.0f)) * rotate( 40.0f*float(M_PI)/180.0f, vec3(0.0f, 1.0f, 0.0f) ));
    ...
    node["world"] -> modeltransforms.push_back( translate(vec3(0.0f,0.5f,0.0f)) * scale(vec3(0.5f)) );
